...
export function getPaginationModel(
  ...
  const boundaryPagesRange =
    options.boundaryPagesRange == null ? 1 : Number(options.boundaryPagesRange);
  ...
  const siblingPagesRange =
    options.siblingPagesRange == null ? 1 : Number(options.siblingPagesRange);
  ...
  const hidePreviousAndNextPageLinks = Boolean(
    options.hidePreviousAndNextPageLinks
  );
...
} from './ultimate-pagination-constants';
